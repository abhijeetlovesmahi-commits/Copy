/* THE LALIT INTERNATIONAL SCHOOL - UPDATED SECURE MODULAR MENU */

// 1. Security Guard: Redirect to login if not authenticated
firebase.auth().onAuthStateChanged((user) => {
    const currentPage = window.location.pathname.split("/").pop();
    // Allow access to login and reset pages without authentication
    if (!user && currentPage !== 'login.html' && currentPage !== 'reset.html') {
...
async function initUniversalMenu() {
    // Current User aur Role Fetch karna
    const user = firebase.auth().currentUser;
    let userRole = 'teacher'; // Default role safely

    if(user) {
        // Firestore se user ka role check karna (Agar 'users' collection banaya hai)
        try {
            const userDoc = await firebase.firestore().collection('users').doc(user.uid).get();
            if(userDoc.exists) {
                userRole = userDoc.data().role;
            } else if (user.email === "admin@lalit.com") { // Fail-safe for super admin
                userRole = 'admin';
            }
        } catch(e) { console.log("Role fetch error, defaulting to teacher."); }
    }
    ...
    menuConfig.forEach(item => {
        if (item.type === "divider") {
            // Check if divider has items to show for this role
            menuItemsHTML += `<div class="menu-divider">${item.title}</div>`;
        } else {
            // Role Based Visibility Logic
            if(item.role === "all" || item.role === userRole) {
    ...
    menuWrapper.innerHTML = `
        ...
        <div id="sidebar-panel">
            <div class="sidebar-header">
                ...
                ${user ? `<div class="user-badge">${user.email.split('@')[0].toUpperCase()}</div>` : ''}
            </div>
    ...
    // CSS Styling (Pehle wali + User Badge)
    ...
    style.textContent = `
        ...
        .user-badge { font-size: 10px; background: #D4AF37; color: #002366; display: inline-block; padding: 2px 10px; border-radius: 10px; margin-top: 10px; font-weight: 800; }
    ...
    if(logoutBtn) {
        logoutBtn.onclick = (e) => {
            e.preventDefault();
            if(confirm("Do you want to exit Control Tower?")) {
...
// Initialization with Auth state
